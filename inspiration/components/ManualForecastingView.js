// src/components/ManualForecastingView.js
...
import { useAuth } from '../context/AuthContext'; // To get currentUser for permissions
 ...
 */
const simpleLinearRegression = (y, x) => {
  const n = y.length;
  ...
  const intercept = (sum_y - slope * x[0]) / n;
...
function ManualForecastingView() {
  const { currentUser, loadingAuth } = useAuth(); // Get currentUser and loadingAuth from AuthContext
  const { showMessageBox, showToast } = useMessageBox(); // Use the custom message box
  ...
  const [selectedForecastProduct, setSelectedForecastProduct] = useState(null);
  ...
  const processedForecastData = useMemo(() => {
    ...
    try {
      ...
      showToast('Error processing CSV data: ' + e.message, 'error');
      console.error(e);
      return {};
    }
  ...
  const handleManualForecast = () => {
    ...
    try {
      ...
      const combinedChartData = historicalData.map(d => ({
      ...
      showToast('Failed to generate manual forecast. ' + e.message, 'error');
    } finally {
