...
import { useRefurbishedDevices } from '../hooks/useRefurbishedDevices';
...
function RefurbishedDeviceView() {
  ...
  const { userRoles } = useAuth(); // Get user roles
  ...
  const [deviceToEdit, setDeviceToEdit] = useState<RefurbishedDevice | null>(null);

  // History Modal states
  const [showHistoryModal, setShowHistoryModal] = useState(false);
  const [selectedItemHistory, setSelectedItemHistory] = useState<RefurbishedDevice | null>(null);
  ...
  const canEdit = userRoles?.Inventory === 'Editor'; // Assuming Inventory Editor can edit refurbished devices
  ...
  const sortedDevices = useMemo(() => {
    ...
    filteredItems.sort((a: RefurbishedDevice, b: RefurbishedDevice) => {
      ...
      const result = naturalSort(String(aValue), String(bValue), sortDirection);
  ...
  const handleAddOrUpdateDevice = async (data: Partial<RefurbishedDevice>) => {
    setIsSubmitting(true);
    try {
      if (deviceToEdit) {
        await updateDevice(deviceToEdit.id!, data);
        showToast('Refurbished device updated successfully!', 'success');
      } else {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    // Build a new ordered array based on the current displayed list
    const newOrder = Array.from(sortedDevices);
    const [moved] = newOrder.splice(sourceIndex, 1);
    newOrder.splice(destIndex, 0, moved);

    try {
      await reorderDevices(newOrder as RefurbishedDevice[]);
    } catch (e) {
      console.error('Failed to persist new order:', e);
    }
  };

  const handleDeleteDevice = async (device: RefurbishedDevice) => {
    if (!canEdit) {
    ...
    if (confirmed) {
      if (!device.id) {
        showToast('Cannot delete device without an ID.', 'error');
        return;
      }
      await deleteDevice(device.id);
      showToast('Refurbished device deleted successfully!', 'success');
    }
  };

  const openEditModal = (item: RefurbishedDevice) => {
    if (!canEdit) {
  ...
  const openHistoryModal = async (item: RefurbishedDevice) => {
    setHistoryLoading(true);
    setSelectedItemHistory(item);
    setItemHistoryRecords([]);
    setShowHistoryModal(true);
    try {
      // const history = await fetchDeviceHistory(item.id); // Uncomment when fetchDeviceHistory is available
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-700">
              ...
              <Droppable droppableId="refurbished-devices">
                {provided => (
                  <tbody
                  ...
                  >
                    {sortedDevices.map((device: RefurbishedDevice, index: number) => (
                      <Draggable
                        key={String(device.id ?? `device-${index}`)}
                        draggableId={String(device.id ?? `device-${index}`)}
                        index={index}
                      >
                        {prov => (
                          <tr
