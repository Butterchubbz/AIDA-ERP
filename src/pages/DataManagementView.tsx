...
const DataManagementView = () => {
  ...
  const [labelSize, setLabelSize] = useState<LabelSize>(() => {
    try {
      const saved = localStorage.getItem('aida_label_size') as LabelSize | null;
      return saved ?? '2x1';
    } catch {
      return '2x1';
    }
                ...
                <select
                  ...
                  onChange={e => {
                    ...
                    const firstSupported =
                      PRINTER_PROFILES.find(p => p.type === next)?.supportedLabelSizes[0] ?? '2x1';
            ...
            <div>
              ...
              <div className="rounded-lg border border-dashed border-slate-500 bg-slate-900/60 p-4">
                <div
                  ...
                  style={{
                    width: `${currentDimensions.width * 40}px`,
                    height: `${currentDimensions.height * 40}px`,
                    maxWidth: '100%',
                  }}
                >
                  {labelSize} ({currentDimensions.width}x{currentDimensions.height}
                  {currentDimensions.unit})
                </div>
              </div>
            </div>
          </div>
