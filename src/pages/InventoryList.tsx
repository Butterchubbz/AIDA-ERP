...
import StockCountModal from '../components/modules/StockCountModal';
...
const InventoryList: React.FC<InventoryListProps> = ({ onEditItem, onAddItem }) => {
  const { inventory, deleteInventoryItem, updateInventoryItem } = useInventoryContext();
  ...
  const aValStr = String(aValue ?? '');
  const bValStr = String(bValue ?? '');
  const result = naturalSort(aValStr, bValStr, sortDirection);
        if (result !== 0) return result;
        if (sortColumn !== 'name') {
          return naturalSort(String(a.name ?? ''), String(b.name ?? ''), 'asc');
        }
        return 0;
      });
    }
  ...
  const handleSaveDeviceCounts = async (updates: StockCountUpdate[]) => {
    ...
    try {
      for (const update of updates) {
        // Persist countedStock to the inventory record. InventoryItem includes countedStock as optional.
        await updateInventoryItem(update.id, { countedStock: update.countedStock });
      }
