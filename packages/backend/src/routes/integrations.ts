...
export async function listUserIntegrations(req: Request, res: Response): Promise<void> {
  ...
  try {
    const records = (await pb
      ...
      .getFullList({ filter: pb.filter('userId = {:userId}', { userId: req.user.id }) })) as IntegrationRecord[]
...
export async function listUnknownSkus(req: Request, res: Response): Promise<void> {
  ...
  try {
    const items = await pb.collection('wcUnknownSkus').getFullList({ filter: 'dismissed != true', sort: '-seenAt' })
