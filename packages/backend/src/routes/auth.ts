...
import { ROLE_PERMISSIONS, type LoginRequest, type LoginResponse } from '@aida/shared'
import { getSetupOwnerLockStatus, trackSetupOwnerLogin } from '../lib/setupOwnerLock.js'
import { writeAuditRecord } from '../middleware/audit.js'
...
export async function login(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    // Global lock: if setup owner has been inactive for 90+ days, only setup owner can unlock by logging in.
    const lockStatus = await getSetupOwnerLockStatus(userEmail)
    ...
    try {
      await trackSetupOwnerLogin(userEmail)
    } catch (trackErr: unknown) {
      console.warn('[Auth] Could not update setup-owner login timestamp:', trackErr)
    }
