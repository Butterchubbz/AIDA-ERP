...
export async function getDeviceHistory(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const events = await pb
      ...
      .getFullList({ filter: pb.filter('deviceId = {:deviceId}', { deviceId: itemId }), sort: '-created' })
...
/**
 * GET /api/inventory/items/:id/print-metadata
 * Fetch a sanitized print payload for a label printer.
 */
export async function getItemPrintMetadata(req: Request, res: Response): Promise<void> {
  if (!req.user) {
    res.status(401).json({ error: 'Unauthorized' })
    return
  }
  ...
  try {
    const { id } = req.params
    if (!id) {
      res.status(400).json({ error: 'Missing inventory item id' })
      return
    }

    const collections = ['inventoryDevice', 'inventoryComponent', 'inventoryAccessory']

    let item: Record<string, any> | null = null
    for (const collection of collections) {
      item = await pb.collection(collection).getOne(id).catch(() => null)
      if (item) break
    }
    ...
    if (!item) {
      res.status(404).json({
        ...
        detail: `No inventory record matched id ${id}.`,
      })
      return
    }

    const workspaceId = typeof item.workspaceId === 'string' ? item.workspaceId : item.workspaceId?.id
    let workspaceName = 'Unassigned'

    if (workspaceId) {
      const workspace = await pb.collection('inventoryWorkspaces').getOne(workspaceId).catch(() => null)
      if (workspace?.name) {
    ...
    console.error('[Inventory] GET print metadata failed:', err)
    res.status(500).json({
      error: 'Failed to load print metadata',
      detail: 'A server error occurred while preparing the label payload.',
    })
  }
}
...
export async function searchInventory(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const devices = await pb
      ...
      .getFullList({ filter: pb.filter('sku ~ {:query} || description ~ {:query}', { query: q }) })
...
export async function getAccessoryHistory(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const events = await pb
      .collection('stockHistory')
      .getFullList({ filter: pb.filter('inventoryItemId = {:inventoryItemId}', { inventoryItemId: itemId }), sort: '-created' })
    res.status(200).json(events)
  } catch (err: unknown) {
...
export async function moveSkuToSection(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const existingInTarget = await pb
      ...
      .getFullList({ filter: pb.filter('sku = {:sku}', { sku }), limit: 1 })
