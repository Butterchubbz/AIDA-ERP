...
export async function getUserIntegration(userId: string, type: string): Promise<IntegrationRecord | null> {
  return pb
    ...
    .getFirstListItem(pb.filter('userId = {:userId} && type = {:type}', { userId, type }))
...
export async function runIntegrationSync(
  ...
  try {
    ...
    const inventoryMsg = result.errors.length > 0 ? result.errors.slice(0, 5).join('; ') : null
    const salesMsg = result.salesErrors.length > 0 ? `Sales data unavailable: ${result.salesErrors[0]}` : null
    const combinedMsg = [inventoryMsg, salesMsg].filter(Boolean).join(' | ')
    await pb.collection('integrations').update(record.id, {
      lastSyncAt: new Date().toISOString(),
      lastSyncStatus: result.errors.length === 0 ? 'success' : 'partial',
      lastSyncMessage: combinedMsg || null,
    })
    return result
  } catch (err: unknown) {
