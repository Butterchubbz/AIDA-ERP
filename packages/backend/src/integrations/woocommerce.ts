  ...
  async sync(credentials: Record<string, string>, pb: PocketBase): Promise<SyncResult> {
    ...
    for (const unknownEntry of unknownSkus) {
      try {
        const existing = await pb
          ...
          .getFirstListItem(pb.filter('sku = {:sku}', { sku: unknownEntry.sku }), { requestKey: null })
          .catch(() => null)
    ...
    while (true) {
      ...
      const res = await wcFetch(url, authHeader).catch((err: unknown) => {
        salesErrors.push(`Sales history fetch failed (page ${ordersPage}): ${err instanceof Error ? err.message : 'unknown error'}`)
        return null
      })

      if (!res) break
      if (!res.ok) {
        salesErrors.push(`WooCommerce orders API returned ${res.status} on page ${ordersPage} — skipping sales history`)
        break
      }
    ...
    salesErrors.push(...salesResult.errors)

    return { recordsImported, salesImported: salesResult.upserted, errors, salesErrors, unknownSkuCount: unknownSkus.length }
  },
}
