  ...
  async sync(credentials: Record<string, string>, pb: PocketBase): Promise<SyncResult> {
    ...
    for (const entry of unknownSkus) {
      try {
        const existing = await pb
          ...
          .getFirstListItem(pb.filter('sku = {:sku}', { sku: entry.sku }), { requestKey: null })
          .catch(() => null)
    ...
    while (ordersNextUrl) {
      const res = await shopifyFetch(ordersNextUrl, accessToken).catch((err: unknown) => {
        salesErrors.push(`Sales history fetch failed: ${err instanceof Error ? err.message : 'unknown error'}`)
        return null
      })

      if (!res) break
      if (!res.ok) {
        salesErrors.push(`Shopify orders API returned ${res.status} — skipping sales history`)
        break
      }
