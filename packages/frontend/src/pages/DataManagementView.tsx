...
const DataManagementView = () => {
  ...
  const [ownerLockStatus, setOwnerLockStatus] = useState<OwnerLockStatus | null>(null);
  const [ownerLockLoading, setOwnerLockLoading] = useState(true);
  const [ownerLockError, setOwnerLockError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const loadOwnerLockStatus = async () => {
      setOwnerLockLoading(true);
      setOwnerLockError(null);
      try {
        const status = await apiClient.get<OwnerLockStatus>('/api/setup/owner-lock-status');
        if (active) {
          setOwnerLockStatus(status);
        }
      } catch (error: unknown) {
        if (active) {
          setOwnerLockError((error as { message?: string }).message ?? 'Could not load shutdown timer.');
        }
      } finally {
        if (active) {
          setOwnerLockLoading(false);
        }
      }
    };

    void loadOwnerLockStatus();
    ...
    <div className="bg-slate-800 p-6 rounded-lg shadow-xl text-slate-100">
      ...
      <div className="space-y-6">
        ...
        <section className="border-t border-slate-700 pt-6">
          ...
          <div className="bg-red-900/20 rounded-lg p-5 border border-red-600/50 mb-4">
          ...
          <div className={`rounded-lg p-5 border ${ownerStatusTone}`}>
            <h3 className="text-lg font-semibold mb-2">Setup Owner Activity Timer</h3>
            {ownerLockLoading && <p className="text-sm opacity-90">Checking inactivity shutdown timer...</p>}
            {!ownerLockLoading && ownerLockError && (
              <p className="text-sm">Could not load timer status: {ownerLockError}</p>
            )}
            {!ownerLockLoading && !ownerLockError && ownerLockStatus && (
              <div className="space-y-2 text-sm">
                <p>
                  AIDA auto-shutdown window: {ownerLockStatus.activityWindowDays} days
                  {ownerLockStatus.state === 'warning' &&
                    ` (warning in last ${ownerLockStatus.warningWindowDays} days)`}
                  .
                </p>
                <p>
                  Setup owner: <span className="font-semibold">{ownerLockStatus.ownerEmail ?? 'Not configured yet'}</span>
                </p>
                <p>
                  Last setup-owner login:{' '}
                  <span className="font-semibold">
                    {ownerLockStatus.lastLoginAt
                      ? new Date(ownerLockStatus.lastLoginAt).toLocaleString()
                      : 'Not recorded yet'}
                  </span>
                </p>
                <p>
                  Days remaining before lockout:{' '}
                  <span className="font-semibold">
                    {ownerLockStatus.daysRemaining ?? 'Unknown'}
                  </span>
                </p>
                {ownerLockStatus.state === 'locked' && (
                  <p className="font-semibold">
                    AIDA is currently in lockout state. Setup owner login is required to unlock access.
                  </p>
                )}
              </div>
            )}
