...
export default function IntegrationsView() {
  ...
  const handleSync = async (type: string) => {
    ...
    try {
      ...
      const salesNote = result.salesImported > 0
        ? ` · ${result.salesImported} sales records written.`
        : (result.salesErrors?.length ?? 0) > 0
          ? ` · Sales history unavailable (check WooCommerce API key has Orders read access).`
          : ''
      if (result.errors.length === 0 && result.unknownSkuCount === 0) {
        ...
        setTimedFeedback(type, {
          tone: 'warning',
          message: `Imported ${result.recordsImported} inventory items${salesNote} · ${result.errors.length} inventory warnings.`,
