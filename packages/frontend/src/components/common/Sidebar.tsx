...
const Sidebar = ({ userRoles }: { userRoles: { [key: string]: string } | null }) => {
  ...
  const [isLogisticsOpen, setLogisticsOpen] = useState(isPathActive('/logistics'));
  ...
  useEffect(() => {
    ...
    setLogisticsOpen(isPathActive('/logistics'));
  ...
  return (
    <aside className="w-64 bg-[#142a52] text-white flex-shrink-0 p-4 flex flex-col rounded-xl shadow-2xl border border-blue-900">
          ...
          {userRoles?.['Inbound Shipments'] && userRoles['Inbound Shipments'] !== 'None' && (
            <li>
              <button
                ...
                className={`${baseLinkClasses} justify-between ${
                  isPathActive('/logistics') ? 'text-white' : 'text-slate-300'
                }`}
              >
                <span className="flex items-center">
                  <i className="fas fa-route w-5 h-5 flex-shrink-0 mr-3"></i>
                  <span>Logistics</span>
                </span>
                <i className={`fas fa-chevron-down transition-transform ${isLogisticsOpen ? 'rotate-180' : ''}`}></i>
              </button>
              {isLogisticsOpen && (
                <ul className="mt-2 space-y-2">
                  <li>
                    <NavLink to="/logistics/shipping" className={getSubNavLinkClass}>
      ...
      <div className="mt-auto pt-4 border-t border-slate-700/70">
        <p className="text-[11px] leading-4 text-slate-300/90">
          Made by Patrick Walton with AI
        </p>
        <p className="text-[10px] leading-4 text-slate-400/90">Copyright (c) {currentYear} Patrick Walton</p>
        <p className="text-[10px] leading-4 text-slate-400/90">All rights reserved.</p>
      </div>
    </aside>
  );
};
