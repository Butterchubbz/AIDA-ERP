...
export const MessageBoxProvider = ({ children }: MessageBoxProviderProps) => {
  ...
  const toastTimerRef = useRef<number | null>(null); // Ref to clear timeout for toast
  ...
  const showMessageBox = useCallback(
    (titleText: string, messageText: string, confirmOption: boolean = false) => {
      setTitle(titleText);
  ...
  const handleConfirm = useCallback(() => {
    ...
    if (resolveRef.current) {
      ...
      resolveRef.current = null; // Clear the ref after use
    }
  }, []);
