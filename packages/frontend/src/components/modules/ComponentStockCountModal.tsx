        ...
        <div className="max-h-96 overflow-y-auto">
          {groupedItems && groupedItems.length > 0
            ? groupedItems.map(group => (
                <div key={group.categoryName} className="mb-3">
                  <div className="text-cyan-300 font-semibold py-1 border-b border-slate-700">
                    {group.categoryName}
                  </div>
                  {group.subcategories.map(sub => (
                    <div key={`${group.categoryName}-${sub.subcategoryName}`} className="mt-1">
                      <div className="text-blue-300 text-sm font-medium py-1">
                        {sub.subcategoryName}
                      </div>
                      {sub.items.map(item => (
                        <div
